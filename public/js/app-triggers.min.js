...
c.selectize({persist:!1,maxItems:null,valueField:"id",labelField:"name",searchField:["name"],plugins:["remove_button"]});var d=$("#otherUsers");0<d.length&&d.selectize({persist:!1,valueField:"id",labelField:"name",searchField:["name"],plugins:["remove_button"],onInitialize:function(){var a=d.data("userId");0<a&&this.removeOption(a)}});var f=$("#otherUserGroups");0<f.length&&f.selectize({persist:!1,valueField:"id",labelField:"name",searchField:["name"],plugins:["remove_button"],onInitialize:function(){var a=
f.data("userGroupId");0<a&&this.removeOption(a)}});c=$("#data-accesses");0<c.length&&1===c[0].childNodes.length&&c.parent(".data").hide();$("input:text:visible:first").focus()},install:function(){e.info("views:install");var a=$("#frmInstall");b.appTheme().passwordDetect(a);d(a)}};return{views:f,selectDetect:d,updateSk:function(){$("#content").find("[data-sk]").each(function(){e.info("updateSk");$(this).data("sk",b.sk.get())})},updateFormHash:function(a){e.info("updateFormHash");a=void 0!==a?a.find(".form-action[data-hash]"):
$(".form-action[data-hash]");0<a.length&&a.each(function(){var a=$(this);a.attr("data-hash",SparkMD5.hash(a.serialize(),!1))})},bodyHooks:function(){e.info("bodyHooks");$("body").on("click","button.btn-action[data-onclick][type='button'],li.btn-action[data-onclick],span.btn-action[data-onclick],i.btn-action[data-onclick],.btn-action-pager[data-onclick]",function(){var a=$(this);e.info("handleActionButton: "+a.attr("id"));var c=a.data("onclick").split("/"),d;d=a.data("plugin");d=void 0!==d&&void 0!==
b.appPlugins()[d]?b.appPlugins()[d]:b.appActions();if(2===c.length)d[c[0]][c[1]](a);else d[c[0]](a)}).on("click",".btn-back",function(){var a=b.appRequests();if(0<a.history.length()){e.info("back");var c=a.history.del();a.getActionCall(c,c.callback)}}).on("submit",".form-action",function(a){a.preventDefault();a=$(this);e.info("formAction");var c=a.attr("data-hash"),d=SparkMD5.hash(a.serialize(),!1);if(c===d)b.msg.ok(b.config().LANG[55]);else if(c=a.data("plugin"),c=void 0!==c&&void 0!==b.appPlugins()[c]?
b.appPlugins()[c]:b.appActions(),d=a.data("onsubmit").split("/"),a.find("input[name='sk']").val(b.sk.get()),2===d.length)c[d[0]][d[1]](a);else c[d[0]](a)}).on("click",".btn-help",function(){var a=$(this),a=$("#"+a.data("help")).html();mdlDialog().show({title:b.config().LANG[54],text:a,positive:{title:b.config().LANG[43]}})}).on("reset",".form-action",function(a){a.preventDefault();e.info("reset");a=$(this);a.find("input:text, input:password, input:file, textarea").val("").parent("div").removeClass("is-dirty");
