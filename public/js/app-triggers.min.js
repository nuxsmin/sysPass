...
plugins:["remove_button"]})},g={main:function(a){d.info("views:main");clipboard.isSupported()||b.msg.info(b.config().LANG[65]);$(".btn-menu").click(function(){var a=$(this);"1"===a.attr("data-history-reset")&&b.appRequests().history.reset();b.appActions().doAction({r:a.data("route")},a.data("view"))});if(0===a.data("upgraded"))b.appActions().doAction({r:"account/index"},"search");else{a=$("#content");var c=a.data("page");g.common(a);if(""!==c&&"function"===typeof g[c])g[c]()}!0===b.config().CHECK_UPDATES&&
