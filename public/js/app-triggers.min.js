...
import_defaultgroup:$("#import_defaultgroup").val()})}}},account:function(){e.info("views:account");var a=$("#list-account-files");0<a.length&&b.appActions().account.listFiles(a);var c=$("#drop-account-files");if(0<c.length){var g=b.fileUpload(c);g.url=b.appActions().ajaxUrl.entrypoint+"?r="+c.data("action-route")+"/"+c.data("item-id");g.allowedExts=b.config().FILES_ALLOWED_EXTS;g.requestDoneAction=function(){b.appActions().account.listFiles(a)}}c=$("#selParentAccount");0<c.length&&(c.on("change",
function(){var a=$(this),b=$("#accountpass,#accountpassR");0<a[0].value?b.each(function(){$(this).prop("disabled","true");$(this).prop("required","false")}):b.each(function(){$(this).prop("disabled","");$(this).prop("required","true")})}),b.appActions().items.get(c));c=$("#selTags");0<c.length&&c.selectize({persist:!1,maxItems:null,valueField:"id",labelField:"name",searchField:["name"],plugins:["remove_button"]});var d=$("#otherUsers");0<d.length&&d.selectize({persist:!1,valueField:"id",labelField:"name",
searchField:["name"],plugins:["remove_button"],onInitialize:function(){var a=d.data("userId");0<a&&this.removeOption(a)}});var f=$("#otherUserGroups");0<f.length&&f.selectize({persist:!1,valueField:"id",labelField:"name",searchField:["name"],plugins:["remove_button"],onInitialize:function(){var a=f.data("userGroupId");0<a&&this.removeOption(a)}});$("input:text:visible:first").focus()},install:function(){e.info("views:install");var a=$("#frmInstall");b.appTheme().passwordDetect(a);d(a)}};return{views:f,
