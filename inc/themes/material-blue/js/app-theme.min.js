...
a.passwordData.complexity.symbols=$("#checkbox-symbols").is(":checked");a.passwordData.complexity.numlength=parseInt($("#passlength").val());b.dialog("close")});componentHandler.upgradeDom()})},close:function(){$(this).dialog("destroy")}})},g=function(b){b.find(".passwordfield__input").each(function(){var b=$(this);if("true"!==b.attr("data-pass-upgraded")){var e=b.parent(),c=b.attr("id"),f=$("#"+c+"R"),h='<button id="menu-speed-'+c+'" class="mdl-button mdl-js-button mdl-button--icon" type="button" title="'+
a.config().LANG[27]+'"><i class="material-icons">more_vert</i></button>',h=h+('<ul class="mdl-menu mdl-js-menu" for="menu-speed-'+c+'">')+('<li class="mdl-menu__item passGen"><i class="material-icons">settings</i>'+a.config().LANG[28]+"</li>"),h=h+('<li class="mdl-menu__item passComplexity"><i class="material-icons">vpn_key</i>'+a.config().LANG[29]+"</li>"),h=h+('<li class="mdl-menu__item reset"><i class="material-icons">refresh</i>'+a.config().LANG[30]+"</li>");e.after('<div class="password-actions" />');
e.next(".password-actions").prepend('<span class="passLevel passLevel-'+c+' fullround" title="'+a.config().LANG[31]+'"></span>').prepend('<i class="showpass material-icons" title="'+a.config().LANG[32]+'">remove_red_eye</i>').prepend(h);b.on("keyup",function(){a.checkPassLevel(b)});e=b.parent().next();e.find(".passGen").on("click",function(){d(b);b.focus()});e.find(".passComplexity").on("click",function(){k()});e.find(".showpass").on("mouseover",function(){$(this).attr("title",b.val())});e.find(".reset").on("click",
function(){b.val("");f.val("");componentHandler.upgradeDom()});b.attr("data-pass-upgraded","true")}});b.find(".passwordfield__input-show").each(function(){var b=$(this),e=$('<i class="showpass material-icons" title="'+a.config().LANG[32]+'" data-targetid="'+b.attr("id")+'">remove_red_eye</i>');b.parent().after(e);e.on("mouseover",function(){e.attr("title",b.val())})})},m=function(b){f.info("setupDatePicker");var d={format:"YYYY-MM-DD",lang:a.config().LOCALE.substr(0,2),time:!1,cancelText:a.config().LANG[44],
...
a.on("change",function(){var b;b=e(a.val());a.parent().find("input[name='passworddatechange_unix']").val(b)})})};return{passwordDetect:g,password:d,viewsTriggers:{search:function(){var b=$("#frmSearch"),d=$("#res-content");b.find(".icon-searchfav").on("click",function(){var h=$(this).find("i"),c=b.find("input[name='searchfav']");0==c.val()?(h.addClass("mdl-color-text--amber-A200"),h.attr("title",a.config().LANG[53]),c.val(1)):(h.removeClass("mdl-color-text--amber-A200"),h.attr("title",a.config().LANG[52]),
c.val(0));b.submit()});var e=b.find("#tags")[0],c=b.find(".search-filters-tags"),f=b.find("i.show-filter");d.on("click","#data-search-header .sort-down,#data-search-header .sort-up",function(){var b=$(this);b.parent().find("a").addClass("filterOn");a.appActions().account.sort(b)}).on("click","#search-rows i.icon-favorite",function(){var b=$(this);a.appActions().account.savefavorite(b,function(){"on"===b.data("status")?(b.addClass("mdl-color-text--amber-A100"),b.attr("title",a.config().LANG[50]),b.html("star")):
(b.removeClass("mdl-color-text--amber-A100"),b.attr("title",a.config().LANG[49]),b.html("star_border"))})}).on("click","#search-rows span.tag",function(){c.is(":hidden")&&f.trigger("click");e.selectize.addItem($(this).data("tag-id"))});f.on("click",function(){var a=$(this);c.is(":hidden")?(c.slideDown("slow"),a.html(a.data("icon-up"))):(c.slideUp("slow"),a.html(a.data("icon-down")))});0<e.selectedOptions.length&&f.trigger("click")},common:function(a){g(a);m(a)}},loading:{show:function(){$("#wrap-loading").show();
