...
var d=a.history.del();a.getActionCall(d,d.callback)}}).on("submit",".form-action",function(a){a.preventDefault();a=$(this);c.info("formAction");var d=a.data("onsubmit").split("/"),g=b.appActions(),e=a.attr("data-hash"),f=SparkMD5.hash(a.serialize(),!1);if(e===f)b.msg.ok(b.config().LANG[55]);else if(a.find("input[name='sk']").val(b.sk.get()),2===d.length)g[d[0]][d[1]](a);else g[d[0]](a)}).on("click",".btn-help",function(){var a=$(this),a=$("#"+a.data("help")).html();showDialog({title:b.config().LANG[54],
text:a,positive:{title:b.config().LANG[43]}})}).on("reset",".form-action",function(a){a.preventDefault();c.info("reset");a=$(this);a.find("input:text, input:password, input:file, textarea").val("").parent("div").removeClass("is-dirty");a.find("input:radio, input:checkbox").removeAttr("checked").removeAttr("selected");a.find("input[name='start'], input[name='skey'], input[name='sorder']").val(0);a.find("select").each(function(){$(this)[0].selectize.clear(!0)});a.submit()})};return{views:{main:function(){c.info("views:main");
...
var a=$("#list-account-files");0<a.length&&b.appActions().account.getfiles(a);var d=$("#drop-account-files");0<d.length&&(d=b.fileUpload(d),d.url=b.appActions().ajaxUrl.file,d.requestDoneAction=function(){b.appActions().account.getfiles(a)});d=$(".form-action");0<d.length&&d.attr("data-hash",SparkMD5.hash(d.serialize(),!1));d=$(".show-extra-info");if(0<d.length)d.on("click",function(){var a=$(this),b=$(a.data("target"));0==a.data("state")?(b.show("blind","slow"),a.data("state","1"),a.html(a.data("icon-up"))):
(b.hide("blind","slow"),a.data("state","0"),a.html(a.data("icon-down")))});$("#selParentAccount").on("change",function(){var a=$(this),b=$("#accountpass,#accountpassR");0<a[0].value?b.each(function(){$(this).prop("disabled","true")}):b.each(function(){$(this).prop("disabled","")})})},install:function(){c.info("views:install");f();var a=$("#frmInstall");b.appTheme().passwordDetect(a);e(a)}},selectDetect:e,updateSk:function(){$("#content").find("[data-sk]").each(function(){c.info("updateSk");$(this).data("sk",
b.sk.get())})}}};
