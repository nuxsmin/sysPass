...
final class SyspassImport extends XmlImportBase implements ImportInterface
{
            ...
            if ($this->detectEncrypted()) {
                if ($this->importParams->getImportPwd() === '') {
                    throw new ImportException(
                        __u('Encryption password not set'),
                        SPException::INFO
                    );
                }
    ...
    {
        return VersionUtil::versionToInteger((new DOMXPath($this->xmlDOM))
    ...
    {
        return $this->xmlDOM
                ->getElementsByTagName('Encrypted')->length > 0;
    }
    ...
    protected function processEncrypted(): void
    {
        ...
        foreach ($this->xmlDOM->getElementsByTagName('Data') as $node) {
            ...
            $this->xmlDOM->documentElement
                ->appendChild($this->xmlDOM
                    ->importNode($newXmlData->documentElement, true));
        }
                ...
                foreach ($account->childNodes as $node) {
                    if (isset($node->tagName)) {
                        switch ($node->tagName) {
                            ...
                            case 'tags':
                                $accountRequest->tags =
                                    $this->processAccountTags($node->childNodes);
                        }
                    }
                }
